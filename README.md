...
- **Firebase sync** (optional) — signs in anonymously and automatically (no login screen — there's only one user), syncing your data as a single document in Firestore; the app works fully offline against local storage when Firebase isn't configured.
...
- `@react-native-async-storage/async-storage` for local persistence; the [Firebase JS SDK](https://firebase.google.com/docs/web/setup) (Firestore + Anonymous Auth) for optional cloud sync
...
The app works fully offline on first run — no setup required. It seeds a small library of common exercises and stores everything locally until you optionally connect Firebase.
...
1. Create a project at the [Firebase Console](https://console.firebase.google.com), enable **Firestore Database**, and enable the **Anonymous** provider under **Authentication → Sign-in method**.
2. In Firestore rules, restrict each document to its own device: `allow read, write: if request.auth != null && request.auth.uid == uid;` on `gymTrackerData/{uid}`.
3. Copy `.env.example` to `.env` and fill in the Firebase project config (from Project settings → General → Your apps):
...
Leave these unset to keep the app fully local-only. There's no login screen — once configured, the app signs in anonymously and syncs silently.
...
lib/                    Data model, local storage, PR math, date/streak helpers, Firebase sync
```

## Documentation

[`CLAUDE.md`](./CLAUDE.md) is the living source of truth for this project — product/design decisions and rationale, implementation notes (including a few React Native Web and Firestore gotchas worth knowing before touching styling or sync), and QA verification notes. [`project_instruction.md`](./project_instruction.md) is the original project brief.
